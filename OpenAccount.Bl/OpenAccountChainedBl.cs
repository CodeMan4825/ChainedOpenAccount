...
namespace OpenAccount.Bl
{
	...
	internal abstract class OpenAccountChainedBl<TEntity, TRepository, TKey> : BaseChainOfRespLogic<TEntity, TRepository, TKey, RequestStateType>, IOpenAccountChainedBl<TEntity, TKey>
		...
		where TKey : struct
	{
		...
		protected OpenAccountChainedBl(TRepository logicRepository,
		...
		{	//در اولین مرحله هستیم
			...
			if (log == null)
				throw StException.ChainOfRespLevelViolation(new ValidateExceptionDto(LogicType, "خطا در بازیابی اطلاعات مراحل کار"));
